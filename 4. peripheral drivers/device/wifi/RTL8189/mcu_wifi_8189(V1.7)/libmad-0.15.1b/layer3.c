			      ...
			      unsigned int part2_length)
{
  ...
  exponents = (int*)mem_malloc(39*sizeof(int));
		 ...
		 unsigned char const sfbwidth[39])
{
  ...
  tmp = (mad_fixed_t (*)[32][3][6])mem_malloc(32*3*6*sizeof(mad_fixed_t));
  ...
  //memcpy(&xr[18 * sb], &tmp[sb*18], (576 - 18 * sb) * sizeof(mad_fixed_t));
			  ...
			  unsigned char const *sfbwidth)
{
  ...
  modes = (short*)mem_malloc(39*sizeof(short));
...
void III_imdct_s(mad_fixed_t const X[18], mad_fixed_t z[36])
{
  ...
  /* IMDCT */
  y = (mad_fixed_t*)mem_malloc(36*sizeof(mad_fixed_t));
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  si = (struct sideinfo*)mem_malloc(sizeof(struct sideinfo));
  ...
  if (frame->overlap == 0) {
    frame->overlap = mem_calloc(2 * 32 * 18, sizeof(mad_fixed_t));
    if (frame->overlap == 0) {
      stream->error = MAD_ERROR_NOMEM;
	  mem_free(si);
