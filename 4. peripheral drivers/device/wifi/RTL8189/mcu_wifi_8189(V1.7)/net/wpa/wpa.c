...
extern int wifi_add_key(u8 key_index, bool pairwise, const u8 *mac_addr,
				       ...
				       struct cfg80211_connect_params *sme)
{
	...
	if(sme)
	{
		if(sme->crypto.cipher_group == WLAN_CIPHER_SUITE_CCMP || 
			sme->crypto.cipher_group == WLAN_CIPHER_SUITE_TKIP)
		{
...
uint8_t test11[] = /*{0x30 ,0x14 ,0x01 ,0x00 ,0x00 ,0x0f ,0xac ,0x04 ,0x01 ,0x00 ,0x00 ,0x0f ,0xac ,0x04 ,0x01 ,0x00 ,0x00 ,0x0f ,0xac ,0x02 ,0x00 ,0x00};*/
...
/*
	...
	//sm->assoc_wpa_ie = test11;
	//sm->assoc_wpa_ie_len = sizeof(test11);
				      ...
				      const u8 *key_rsc)
{
	...
	if (sm->pairwise_cipher == WPA_CIPHER_NONE) {
		...
		wifi_add_key(gd->keyidx, 1, broad_addr,
			   &params);
...
/*		if (wpa_sm_set_key(sm, gd->alg,
	...
	else
	{
		...
		//  &params);	
		wifi_add_key(gd->keyidx, (bool)gd->tx, broad_addr,
					   &params);
