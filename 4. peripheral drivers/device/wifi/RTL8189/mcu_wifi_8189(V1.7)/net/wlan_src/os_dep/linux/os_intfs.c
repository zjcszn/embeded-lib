...
const int rtw_chip_version = 0x00;
const int rtw_rfintfs = HWPI;
const int rtw_lbkmode = 0;//RTL8712_AIR_TRX;


const int rtw_network_mode = Ndis802_11IBSS;//Ndis802_11Infrastructure;//infra, ad-hoc, auto
//NDIS_802_11_SSID	ssid;
const int rtw_channel = 6;//ad-hoc support requirement 
const int rtw_wireless_mode = WIRELESS_11BG_24N;
const int rtw_vrtl_carrier_sense = AUTO_VCS;
const int rtw_vcs_type = RTS_CTS;//*
const int rtw_rts_thresh = 2347;//*
const int rtw_frag_thresh = 2346;//*
const int rtw_preamble = PREAMBLE_LONG;//long, short, auto
const int rtw_scan_mode = 1;//active, passive
const int rtw_adhoc_tx_pwr = 1;
const int rtw_soft_ap = 0;
//int smart_ps = 1;  
#ifdef CONFIG_POWER_SAVING
const int rtw_power_mgnt = 1;
#ifdef CONFIG_IPS_LEVEL_2
const int rtw_ips_mode = IPS_LEVEL_2;
#else
const int rtw_ips_mode = IPS_NORMAL;
#endif
#else
const int rtw_power_mgnt = PS_MODE_ACTIVE;
const int rtw_ips_mode = IPS_NONE;
#endif

const int rtw_smart_ps = 2;

#ifdef CONFIG_TX_EARLY_MODE
const int rtw_early_mode=1;
#endif
//module_param(rtw_ips_mode, int, 0644);
//MODULE_PARM_DESC(rtw_ips_mode,"The default IPS mode");

const int rtw_radio_enable = 1;
const int rtw_long_retry_lmt = 7;
const int rtw_short_retry_lmt = 7;
const int rtw_busy_thresh = 40;
//int qos_enable = 0; //*
const int rtw_ack_policy = NORMAL_ACK;

const int rtw_mp_mode = 0;

const int rtw_software_encrypt = 0;
const int rtw_software_decrypt = 0;

const int rtw_acm_method = 0;// 0:By SW 1:By HW.
 
const int rtw_wmm_enable = 1;// default is set to enable the wmm.
const int rtw_uapsd_enable = 0;
const int rtw_uapsd_max_sp = NO_LIMIT;
const int rtw_uapsd_acbk_en = 0;
const int rtw_uapsd_acbe_en = 0;
const int rtw_uapsd_acvi_en = 0;
const int rtw_uapsd_acvo_en = 0;
	
#ifdef CONFIG_80211N_HT
const int rtw_ht_enable = 1;
const int rtw_cbw40_enable = 3; // 0 :diable, bit(0): enable 2.4g, bit(1): enable 5g
const int rtw_ampdu_enable = 1;//for enable tx_ampdu
const int rtw_rx_stbc = 1;// 0: disable, bit(0):enable 2.4g, bit(1):enable 5g, default is set to enable 2.4GHZ for IOT issue with bufflao's AP at 5GHZ
const int rtw_ampdu_amsdu = 0;// 0: disabled, 1:enabled, 2:auto
#endif

const int rtw_lowrate_two_xmit = 1;//Use 2 path Tx to transmit MCS0~7 and legacy mode

//int rf_config = RF_1T2R;  // 1T2R	
const int rtw_rf_config = RF_819X_MAX_TYPE;  //auto
const int rtw_low_power = 0;
#ifdef CONFIG_WIFI_TEST
const int rtw_wifi_spec = 1;//for wifi test
#else
const int rtw_wifi_spec = 0;
#endif
const int rtw_channel_plan = RT_CHANNEL_DOMAIN_MAX;

#ifdef CONFIG_BT_COEXIST
const int rtw_btcoex_enable = 1;
const int rtw_bt_iso = 2;// 0:Low, 1:High, 2:From Efuse
const int rtw_bt_sco = 3;// 0:Idle, 1:None-SCO, 2:SCO, 3:From Counter, 4.Busy, 5.OtherBusy
const int rtw_bt_ampdu =1 ;// 0:Disable BT control A-MPDU, 1:Enable BT control A-MPDU.
#endif

const int rtw_AcceptAddbaReq = _TRUE;// 0:Reject AP's Add BA req, 1:Accept AP's Add BA req.

const int rtw_antdiv_cfg = 2; // 0:OFF , 1:ON, 2:decide by Efuse config
const int rtw_antdiv_type = 0 ; //0:decide by efuse  1: for 88EE, 1Tx and 1RxCG are diversity.(2 Ant with SPDT), 2:  for 88EE, 1Tx and 2Rx are diversity.( 2 Ant, Tx and RxCG are both on aux port, RxCS is on main port ), 3: for 88EE, 1Tx and 1RxCG are fixed.(1Ant, Tx and RxCG are both on aux port)


#ifdef CONFIG_USB_AUTOSUSPEND
const int rtw_enusbss = 1;//0:disable,1:enable
#else
const int rtw_enusbss = 0;//0:disable,1:enable
#endif

const int rtw_hwpdn_mode=2;//0:disable,1:enable,2: by EFUSE config

#ifdef CONFIG_HW_PWRP_DETECTION
const int rtw_hwpwrp_detect = 1;
#else
const int rtw_hwpwrp_detect = 0; //HW power  ping detect 0:disable , 1:enable
#endif

#ifdef CONFIG_USB_HCI
const int rtw_hw_wps_pbc = 1;
#else
const int rtw_hw_wps_pbc = 0;
#endif

#ifdef CONFIG_TX_MCAST2UNI
const int rtw_mc2u_disable = 0;
#endif	// CONFIG_TX_MCAST2UNI

#ifdef CONFIG_DUALMAC_CONCURRENT
const int rtw_dmsp = 0;
#endif	// CONFIG_DUALMAC_CONCURRENT

#ifdef CONFIG_80211D
const int rtw_80211d = 0;
#endif

#ifdef CONFIG_REGULATORY_CTRL
const int rtw_regulatory_id =2;
#else
const int rtw_regulatory_id = 0xff;// Regulatory tab id, 0xff = follow efuse's setting
#endif
//module_param(rtw_regulatory_id, int, 0644);


#ifdef CONFIG_SPECIAL_SETTING_FOR_FUNAI_TV
const int rtw_force_ant = 2;//0 :normal, 1:Main ant, 2:Aux ant
const int rtw_force_igi =0;//0 :normal
//module_param(rtw_force_ant, int, 0644);
//module_param(rtw_force_igi, int, 0644);
#endif

const char* const ifname = "wlan%d";
//module_param(ifname, charp, 0644);
//MODULE_PARM_DESC(ifname, "The default name to allocate for first interface");

const char* const if2name = "wlan%d";
//module_param(if2name, charp, 0644);
//MODULE_PARM_DESC(if2name, "The default name to allocate for second interface");

const char* const rtw_initmac = 0;  // temp mac address if users want to use instead of the mac address in Efuse
...
#ifdef CONFIG_IOL
const int rtw_fw_iol=1;// 0:Disable, 1:enable, 2:by usb speed
//module_param(rtw_fw_iol, int, 0644);
//MODULE_PARM_DESC(rtw_fw_iol,"FW IOL");
#endif //CONFIG_IOL

#ifdef CONFIG_FILE_FWIMG
const char *const rtw_fw_file_path= "";
...
_func_enter_;
	...
	snprintf((char*)registry_par->ifname, 16, "%s", (char*)ifname);
	snprintf((char*)registry_par->if2name, 16, "%s", (char*)if2name);
...
unsigned int rtw_classify8021d(struct sk_buff *skb)
{
    ...
    eth_type = htons(skb->protocol);
	switch (eth_type) {
	case ETH_P_IP:
		dscp = _ip_hdr(skb)->tos & 0xfc;
		break;
	default:
		return 0;
	}
...
int set_smart_link_recv_hook(struct net_device *pnetdev, void* callback)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(pnetdev);

	padapter->recv_hook = (smart_link_recv_hook)callback;
...
u32 rtw_start_drv_threads(_adapter *padapter)
{
...
#endif
	thread_create((void (*)(void*))rtw_xmit_thread, padapter, RTW_XMIT_THREAD_PRIO, 0, RTW_XMIT_THREAD_STACK_SIZE, "RTW_XMIT_THREAD");
	...
	kthread_run(rtw_recv_thread, padapter, RTW_RECV_THREAD_PRIO,0, RTW_RECV_THREAD_STACK_SIZE,  "RTW_RECV_THREAD");
#endif
	...
	{
		thread_create((void (*)(void*))rtw_cmd_thread, padapter, RTW_CMD_THREAD_PRIO, 0, RTW_CMD_THREAD_STACK_SIZE, "RTW_CMD_THREAD");
		_rtw_down_sema(&padapter->cmdpriv.terminate_cmdthread_sema); //wait for cmd_thread to run
	}	
		

#ifdef CONFIG_EVENT_THREAD_MODE
	thread_create(event_thread, padapter, RTW_EVENT_THREAD_STACK_SIZE, 0, 512,  "RTW_EVENT_THREAD");
