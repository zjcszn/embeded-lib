	...
	{		
		...
		{//set wps_ie	
			...
			while( cnt < ielen )
			{
				...
				{
                    int wps_ielen = ( (buf[cnt+1]+2) < (MAX_WPA_IE_LEN<<2)) ? (buf[cnt+1]+2):(MAX_WPA_IE_LEN<<2);
					DBG_871X("SET WPS_IE\n");
					...
					padapter->securitypriv.wps_ie_len = wps_ielen;
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE) == _TRUE)
	{
		...
		prates = (NDIS_802_11_RATES_EX*)&pcur_bss->SupportedRates;
...
static void macstr2num(u8 *dst, u8 *src)
{
	int	jj, kk;
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	DBG_871X( "[%s] persistent_supported = %d\n", __FUNCTION__, pwdinfo->persistent_supported );
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	while(1)
	{
		...
		if ( (p2pie=rtw_get_p2p_ie( &pnetwork->network.IEs[12], pnetwork->network.IELength - 12, NULL, &p2pielen)) )
		{
			...
			{
				//	Handle the P2P Device Info attribute of probe response
				DBG_871X( "[%s] P2P_ATTR_DEVICE_INFO \n", __FUNCTION__ );
				if ( _rtw_memcmp( attr_content, peerMAC, ETH_ALEN ) )
				{
					uintPeerChannel = pnetwork->network.Configuration.DSConfig;
					break;
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	DBG_871X( "[%s] session available = %d\n", __FUNCTION__, pwdinfo->session_available );
	...
	switch(major_cmd)
	{
		...
		case 0x7F:
			switch(minor_cmd)
			{
				...
				{
					if(arg == 0xff){
						DBG_871X("ODM_COMP_DIG\t\tBIT0\n");
						DBG_871X("ODM_COMP_RA_MASK\t\tBIT1\n");
						DBG_871X("ODM_COMP_DYNAMIC_TXPWR\tBIT2\n");
						DBG_871X("ODM_COMP_FA_CNT\t\tBIT3\n");
						DBG_871X("ODM_COMP_RSSI_MONITOR\tBIT4\n");
						DBG_871X("ODM_COMP_CCK_PD\t\tBIT5\n");
						DBG_871X("ODM_COMP_ANT_DIV\t\tBIT6\n");
						DBG_871X("ODM_COMP_PWR_SAVE\t\tBIT7\n");
						DBG_871X("ODM_COMP_PWR_TRAIN\tBIT8\n");
						DBG_871X("ODM_COMP_RATE_ADAPTIVE\tBIT9\n");
						DBG_871X("ODM_COMP_PATH_DIV\t\tBIT10\n");
						DBG_871X("ODM_COMP_PSD	\tBIT11\n");
						DBG_871X("ODM_COMP_DYNAMIC_PRICCA\tBIT12\n");
						DBG_871X("ODM_COMP_RXHP\t\tBIT13\n");
						DBG_871X("ODM_COMP_EDCA_TURBO\tBIT16\n");
						DBG_871X("ODM_COMP_EARLY_MODE\tBIT17\n");
						DBG_871X("ODM_COMP_TX_PWR_TRACK\tBIT24\n");
						DBG_871X("ODM_COMP_RX_GAIN_TRACK\tBIT25\n");
						DBG_871X("ODM_COMP_CALIBRATION\tBIT26\n");						
...
static void printdata(u8 *pbuf, u32 len)
{
	...
	for (i = 0; (i+4) <= len; i+=4) {
		DBG_871X("%08X", *(u32*)(pbuf + i));
		if ((i+4) & 0x1F) DBG_871X(" ");
		else DBG_871X("\n");
	}
...
#endif
const static iw_handler rtw_handlers[] =
{
	NULL,					/* SIOCSIWCOMMIT */
...
// Set Channel depend on the country code
	...
	/*rtw_mp_efuse_set*/0,				//0x1A
	/*rtw_mp_efuse_get*/0,				//0x1B
...
const struct iw_handler_def rtw_handlers_def =
{
	rtw_handlers,
