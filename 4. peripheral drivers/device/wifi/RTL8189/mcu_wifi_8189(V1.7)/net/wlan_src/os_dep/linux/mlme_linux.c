...
extern void wpa_supplicant_event_assoc(struct wpa_supplicant *wpa_s, struct cfg80211_connect_params *smee);
void rtw_os_indicate_connect(_adapter *adapter)
{
	...
	wpa_supplicant_event_assoc(&wpa_s, adapter->sme);
	
	if(adapter->sme){
		mem_free(adapter->sme);
		adapter->sme = 0;
	}
	_func_exit_;	
...
_func_enter_;
	...
	wpa_supplicant_event_disassoc(&wpa_s,  0);
#ifdef RTK_DMP_PLATFORM
