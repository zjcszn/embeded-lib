...
u32 ffaddr2deviceId(struct dvobj_priv *pdvobj, u32 addr)
{
...
#endif
extern struct xmit_buf*	dequeue_pending_xmitbuf(struct xmit_priv *pxmitpriv);
extern struct xmit_buf*	dequeue_pending_xmitbuf_under_survey(struct xmit_priv *pxmitpriv);
...
free_xmitbuf:		
	...
	schedule_tasklet(&pxmitpriv->xmit_tasklet);
#endif
...
#endif
DECLARE_MONITOR_ITEM("frame max len", frame_max_len);
#ifdef CONFIG_SDIO_TX_TASKLET
...
#if SYS_MONITOR
	if(total_len < GET_MONITOR_ITEM_VALUE(frame_max_len))
		SET_MONITOR_ITEM_VALUE(frame_max_len, total_len);
#endif
...
s32 rtl8188es_init_xmit_priv(PADAPTER padapter)
{
	...
	init_tasklet(&pxmitpriv->xmit_tasklet,
