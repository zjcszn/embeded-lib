...
static u8 _CardEnable(PADAPTER padapter)
{
	...
	if (bMacPwrCtrlOn == _FALSE)
	{
		...
		ret = HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_SDIO_MSK, (WLAN_PWR_CFG*)Rtl8188E_NIC_ENABLE_FLOW);
		if (ret == _SUCCESS) {
...
static void CardDisableRTL8188ESdio(PADAPTER padapter)
{
	...
	ret = HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_SDIO_MSK, (WLAN_PWR_CFG*)Rtl8188E_NIC_LPS_ENTER_FLOW);
	if (ret == _FALSE) {
...
*/	
	{ // Non-Power Down
		...
		// Card disable power action flow
		ret = HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_SDIO_MSK, (WLAN_PWR_CFG*)Rtl8188E_NIC_DISABLE_FLOW);
...
DBG_871X("hw_var_set_bssid   reg=%x \n", reg_bssid);
		...
		case HW_DEF_ODM_DBG_FLAG:
			{
				...
				DBG_871X("pDM_Odm->DebugComponents = 0x%llx \n",pDM_Odm->DebugComponents );			
			}
			break;	
		default:
