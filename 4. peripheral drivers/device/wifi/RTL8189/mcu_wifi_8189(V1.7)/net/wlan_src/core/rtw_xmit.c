...
void  rtw_mfree_xmit_priv_lock (struct xmit_priv *pxmitpriv)
{
	...
	rtw_sctx_deinit(&pxmitpriv->ack_tx_ops);	
...
//	u32 max_xmit_extbuf_size = MAX_XMIT_EXTBUF_SZ;
	u32 num_xmit_extbuf = NR_XMIT_EXTBUFF;
	num_xmit_extbuf=num_xmit_extbuf;
#if defined(CONFIG_MP_INCLUDED) && defined(CONFIG_RTL8723A)
	if (padapter->registrypriv.mp_mode) {
			max_xmit_extbuf_size = 6000;
			num_xmit_extbuf = 8;
...
static s32 update_attrib(_adapter *padapter, _pkt *pkt, struct pkt_attrib *pattrib)
{
 ...
 _func_enter_;
	...
	pattrib->ether_type = __swab16(etherhdr.h_proto);
		...
		if (pktfile.pkt_len > 282) {//MINIMUM_DHCP_PACKET_SIZE) {
			if (ETH_P_IP == pattrib->ether_type) {// IP header
				if (((tmp[21] == 68) && (tmp[23] == 67)) ||
					...
					// 68 : UDP BOOTP client
					// 67 : UDP BOOTP server
					RT_TRACE(_module_rtl871x_xmit_c_,_drv_info_,("======================update_attrib: get DHCP Packet \n"));
...
s32 rtw_make_wlanhdr (_adapter *padapter , u8 *hdr, struct pkt_attrib *pattrib)
{
	...
	sint res = _SUCCESS;
	u16 *fctrl = (u16 *)&pwlanhdr->frame_ctl;
...
_func_enter_;
	...
	switch(pxmitpriv->vcs_setting)
	{
		...
		case AUTO_VCS:
		default:
			perp = rtw_get_ie(ie, _ERPINFO_IE_, (uint*)&erp_len, ie_len);
			if(perp == NULL)
			{
				pxmitpriv->vcs = NONE_VCS;
			}
...
#ifdef XMIT_BUFF_ALLOC_DYNAMIC
	rtw_os_xmit_resource_alloc(pxmitpriv->adapter, pxmitbuf, MAX_XMIT_EXTBUF_SZ);
	if(!pxmitbuf->pallocated_buf)
	{
...
#ifdef XMIT_BUFF_ALLOC_DYNAMIC
	rtw_os_xmit_resource_free(pxmitpriv->adapter, pxmitbuf, MAX_XMITBUF_SZ);
#endif
...
#ifdef XMIT_BUFF_ALLOC_DYNAMIC
	rtw_os_xmit_resource_alloc(pxmitpriv->adapter, pxmitbuf, MAX_XMITBUF_SZ);
	if(!pxmitbuf->pallocated_buf)
	{
		rtw_free_xmitbuf_ext(pxmitpriv, pxmitbuf);
		pxmitbuf = 0;
...
void rtw_sctx_init(struct submit_ctx *sctx, int timeout_ms)
{
	...
	init_waitqueue_head(&sctx->done);
#endif
	sctx->status = RTW_SCTX_SUBMITTED;
}

void rtw_sctx_deinit(struct submit_ctx *sctx)
{
	del_waitqueue_head(&sctx->done);
}
...
#ifdef PLATFORM_LINUX
	expire= sctx->timeout_ms ? /*msecs_to_jiffies*/(sctx->timeout_ms) : 0;
	if (wait_event_timeout(sctx->done, expire)) {
