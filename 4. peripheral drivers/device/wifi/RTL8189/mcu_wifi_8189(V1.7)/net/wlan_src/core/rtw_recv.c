...
u32 rtw_free_uc_swdec_pending_queue(_adapter *adapter)
{
	...
	while((pending_frame=rtw_alloc_recvframe(&adapter->recvpriv.uc_swdec_pending_queue)) != 0) {
...
_func_enter_;
	...
	if(auth_alg==2)
	{
		if ((psta!=NULL) && (psta->ieee8021x_blocked))
		{
			...
			ether_type= __swab16((unsigned short )ether_type);
...
_func_enter_;
		...
		)
	{
		     ...
		     (!_rtw_memcmp(pattrib->bssid, mybssid, ETH_ALEN)) )
		{
			...
			DBG_871X( "this adapter = %d, buddy adapter = %d\n", /*adapter->adapter_type*/0, /*adapter->pbuddy_adapter->adapter_type*/0 );
			#endif
...
exit:
	...
	if(adapter->recv_hook)
	{
		adapter->recv_hook(pattrib->src, pattrib->pkt_len);
	}
...
_func_enter_;
	...
	eth_type= __swab16((unsigned short )eth_type); //pattrib->ether_type
...
#else
	ppkt = __netdev_alloc_skb(padapter->pnetdev, alloc_sz, 0);
#endif
...
int amsdu_to_msdu(_adapter *padapter, union recv_frame *prframe)
{
...
#ifdef ENDIAN_FREE
		//nSubframe_Length = __swab16(*((u16*)(pdata + 12)));
		nSubframe_Length = RTW_GET_BE16(pdata + 12);
#else // ENDIAN_FREE
		nSubframe_Length = *((u16*)(pdata + 12));
		//==m==>change the length order
		nSubframe_Length = (nSubframe_Length>>8) + (nSubframe_Length<<8);
		//__swab16(nSubframe_Length);
#endif // ENDIAN_FREE
	...
	for(i=0; i<nr_subframes; i++){
		...
		/* convert hdr + possible LLC headers into Ethernet header */
#ifdef ENDIAN_FREE
		//eth_type = __swab16(*(u16*)&sub_skb->data[6]);
		...
		/* convert hdr + possible LLC headers into Ethernet header */
#ifdef ENDIAN_FREE
		eth_type = __swab16(*(u16*)&ptr[offset+6]);
#else // ENDIAN_FREE
...
exit:
	...
	do{
		...
		type_len= __swab16((unsigned short )type_len);
		ptr +=2;
		mv_len += ETH_HLEN;
...
pattrib=pattrib;
	...
	if (ret != _SUCCESS)
	{
		...
		//RT_TRACE(_module_rtl871x_recv_c_, _drv_info_, ("recv_func: validate_recv_frame ok!\n"));
...
pattrib=pattrib;
	...
	if (prframe == NULL) {
		RT_TRACE(_module_rtl871x_recv_c_,_drv_warning_,("decryptor: drop pkt\n"));
		#ifdef DBG_RX_DROP_FRAME
...
recvpriv=recvpriv;
	...
	if (check_fwstate(mlmepriv, WIFI_STATION_STATE) && psecuritypriv->busetkipkey)
	{
		...
		while((pending_frame=rtw_alloc_recvframe(&padapter->recvpriv.uc_swdec_pending_queue)) != 0) {
...
num_signal_qual=num_signal_qual;
