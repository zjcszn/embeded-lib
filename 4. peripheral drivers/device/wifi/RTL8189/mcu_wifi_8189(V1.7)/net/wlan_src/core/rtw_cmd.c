...
_next:
		...
		if((pcmd = rtw_dequeue_cmd(pcmdpriv)) == 0) {
#ifdef CONFIG_LPS_LCLK
...
_func_enter_;
    ...
    if(psurveyPara->scan_mode == SCAN_ACTIVE)   //add by lck
        pmlmepriv->scan_ssid = *ssid;
    else
        memset(&pmlmepriv->scan_ssid, 0, sizeof(NDIS_802_11_SSID));
	...
	if(pregistrypriv->ht_enable)
	{
			...
			( padapter->securitypriv.dot11PrivacyAlgrthm != _TKIP_ ))
		{
			...
			rtw_restructure_ht_ie(padapter, &pnetwork->network.IEs[0], &psecnetwork->IEs[0], 
									pnetwork->network.IELength, (uint*)&psecnetwork->IELength);			
		}
	}
