...
#define	INIT_TMR()					/* Always: Initialize Timers, Ports */
...
#define CAPT_TST()					/* RX: Check which edge triggered the capture event (true:IR start) */
#define	CAPT_VAL()					/* RX: Get the captured timer value */
#define CAPT_ENA()					/* RX: Enable capture interrupt */
#define CAPT_DIS()					/* RX && TX: Disable capture interrupt */

#define COMP_ENA(n)					/* Always: Enable compare interrupt n count after now */
#define COMP_DIS()					/* Always: Disable compare interrupt */
#define COMP_NXT(n)					/* TX: Increase compare register by n count */

/* Counter clock rate and register width */
#define T_CLK						/* Always: Timer tick period [ns] */
#define	irtmr_t						/* Always: Integer type of timer counter */

/* Definitions of interrupt service functions */
#define	ISR_COMPARE()				/* Always: Timer compare match ISR */
#define	ISR_CAPTURE()				/* RX: Timer input capture ISR */
