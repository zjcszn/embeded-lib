...
{
//		Write_Reg(ComIEnReg, 0x00 ); //ComIEnReg	0x02
//	Write_Reg(DivIEnReg, 0x00 );//DivIEnReg	0x03
//		Write_Reg(ComIrqReg	, 0x3f );//ComIrqReg	0x04
//	Clear_BitMask(Status2Reg,0x08);//Status2Reg	0x08
//	Write_Ext_Reg(0x01,0x21);//
//		Write_Ext_Reg(0x03,0x04);//
//		Write_Reg(RxSelReg	, 0x88);// RxSelReg 0x17
...
//	  Write_Reg(TxAutoReg, 0x24);// 金融过检设置TxASKReg 0x15 设置100%ASK无效	
//		Write_Reg(TxBitPhaseReg, 0x8f);// 金融过检设置TxBitPhaseReg 0x25 
//		Write_Reg(GsNOnReg, 0xf7);// 金融过检设置GsNOnReg 0x27
//		Write_Reg(GsNOffReg, 0xff);// 金融过检设置 GsNOffReg 0x23
//		Write_Reg(TYPEBREG, 0xc0);// 金融过检设置 TYPEBREG  0x1E 
//		 Write_Reg(ControlReg, 0x10);// 金融过检设置 ControlReg	0x0C
//		Write_Reg(DemodReg, 0x5D);// 金融过检设置 DemodReg 0x19
		Write_Reg(BitFramingReg, 0x00);// 金融过检设置 BitFramingReg 0x0D  不加这个寄存器设置 typer A  B轮询即有问题寻卡失败
		 
		 //CWGsNOn/ModGsNOn// 低四位改变调制深度，数值越小，调制深度越小；
		 
		Write_Reg(RFCfgReg,0x58);  //0x26 6-4 RxGain 接收增益  0x08/0x38 18db 0x18/0x48 33db 0x58 38db 0x68 43db 0x78 48db
		Write_Reg(GsNOnReg, 0xF4); //0x27 7-4 CWGsNOn 无调制N电导 3-0 ModGsNOn 调制时N电导，深度
		Write_Reg(CWGsPReg,0x3F);  //0x28 5-0 CWGsP 无调制时PMOS的输出
		Write_Reg(ModGsPReg,0x20); //0x29 5-0 ModGsP 调制时PMOS的输出，可以控制调制深度
		Write_Reg(RxThresholdReg,0x84);//0x18 7-4 MinLevel译码器最小信号强度，低于不处理 2-0 CollLevel
	}
   return OK;
}
/*********************************************/
