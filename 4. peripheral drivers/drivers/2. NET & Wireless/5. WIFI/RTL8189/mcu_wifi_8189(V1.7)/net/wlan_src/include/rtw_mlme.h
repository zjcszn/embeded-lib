...
typedef struct {
	void  *next;
	UCHAR	used;
	struct scan_result_data res_data;
}BSS_ENTRY_SIMPLE;

typedef struct {
    BSS_ENTRY_SIMPLE   *BssEntry;
} BSS_TABLE_SIMPLE, *PBSS_TABLE_SIMPLE;
...
struct mlme_priv {
	...
	BSS_TABLE_SIMPLE ScanTabSimple;	//add by lck
	u8		*free_bss_buf;
	u32	num_of_scanned;

	NDIS_802_11_SSID	assoc_ssid;
	u8	assoc_bssid[6];
    NDIS_802_11_SSID	scan_ssid;  //add by lck
