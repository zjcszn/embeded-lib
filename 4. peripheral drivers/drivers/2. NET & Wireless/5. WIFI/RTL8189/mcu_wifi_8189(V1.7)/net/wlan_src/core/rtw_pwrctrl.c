	...
	if(padapter->pwrctrlpriv.bHWPwrPindetect) 
	{
		...
		if(padapter->registrypriv.usbss_enable)
		{
			if(pwrpriv->rf_pwrstate == rf_on)
			{
				...
				rfpwrstate = /*RfOnOffDetect(padapter)*/0;
                p_err_miss;
...
void register_task_alive(struct pwrctrl_priv *pwrctrl, u32 tag)
{
	pwrctrl->alives |= tag;
}

void unregister_task_alive(struct pwrctrl_priv *pwrctrl, u32 tag)
{
	pwrctrl->alives &= ~tag;
}
