...
const u4Byte Array_AGC_TAB_1T_8188E[] = { 
		0xC78, 0xFB000001,		
		0xC78, 0xFB010001,
		0xC78, 0xFB020001,
		0xC78, 0xFB030001,
 	...
 	)
{
	...
	pu4Byte    Array       = (u32*)Array_AGC_TAB_1T_8188E;
	BOOLEAN		biol = FALSE;
#ifdef CONFIG_IOL_IOREG_CFG
	...
	if(biol){		
		if((pxmit_frame= rtw_IOL_accquire_xmit_frame(Adapter)) == NULL){
			DBG_871X("rtw_IOL_accquire_xmit_frame failed\n");
	...
	if(biol){
		...
		{			
			...
			DBG_871X("~~~ %s IOL_exec_cmds Failed !!! \n",__FUNCTION__);
			#ifdef CONFIG_IOL_IOREG_CFG_DBG
			{
				//dump data from TX packet buffer				
				rtw_IOL_cmd_tx_pkt_buf_dump(pDM_Odm->Adapter,pxmit_frame->attrib.pktlen+32);
...
const u4Byte Array_PHY_REG_1T_8188E[] = { 
		0x800, 0x80040000,			
		0x804, 0x00000003,
		0x808, 0x0000FC00,
		0x80C, 0x0000000A,
 	...
 	)
{
	...
	pu4Byte    Array       = (u32*)Array_PHY_REG_1T_8188E;
	BOOLEAN		biol = FALSE;
#ifdef CONFIG_IOL_IOREG_CFG
	PADAPTER	Adapter =  pDM_Odm->Adapter;	
	struct xmit_frame	*pxmit_frame;
	...
	if(biol){
		...
		{			
			#ifdef CONFIG_IOL_IOREG_CFG_DBG
			DBG_871X("~~~ %s IOL_exec_cmds Success !!! \n",__FUNCTION__);
			{
				u4Byte idx;
				u4Byte cdata;
				DBG_871X("  %s data compare => array_len:%d \n",__FUNCTION__,cmpdata_idx);
				DBG_871X("### %s data compared !!###\n",__FUNCTION__);
				for(idx=0;idx< cmpdata_idx;idx++)
				{
					cdata = ODM_Read4Byte(pDM_Odm, cmpdata[idx].addr);
					if(cdata != cmpdata[idx].value){
						DBG_871X(" addr:0x%04x, data:(0x%02x : 0x%02x) \n",
...
const u4Byte Array_PHY_REG_PG_8188E[] = { 
 	...
 	)
{
	...
	pu4Byte    Array       = (u32*)Array_PHY_REG_PG_8188E;
	BOOLEAN		biol = FALSE;
biol=biol;
ptr_array=ptr_array;
count=count;
