...
const u4Byte Array_MAC_REG_8188E[] = {
		0x026, 0x00000041,		
		0x027, 0x00000035,
		0x428, 0x0000000A,
		0x429, 0x00000010,
 	...
 	)
{
	...
	pu4Byte    Array       = (u32*)Array_MAC_REG_8188E;
	BOOLEAN		biol = FALSE;	
...
ptr_array=ptr_array;
	...
	if(biol){
		...
		{			
			#ifdef CONFIG_IOL_IOREG_CFG_DBG			
			DBG_871X("~~~ IOL Config MAC Success !!! \n");
			//compare writed data
			{
				u4Byte idx;
				u1Byte cdata;
				// HAL_STATUS_FAILURE;
				DBG_871X("  MAC data compare => array_len:%d \n",cmpdata_idx);
				for(idx=0;idx< cmpdata_idx;idx++)
				{
					cdata = ODM_Read1Byte(pDM_Odm, cmpdata[idx].addr);
					if(cdata != cmpdata[idx].value){
						DBG_871X("### MAC data compared failed !! addr:0x%04x, data:(0x%02x : 0x%02x) ###\n",
			...
			DBG_871X("~~~ MAC IOL_exec_cmds Failed !!! \n");
			#ifdef CONFIG_IOL_IOREG_CFG_DBG
			{
				//dump data from TX packet buffer				
				rtw_IOL_cmd_tx_pkt_buf_dump(pDM_Odm->Adapter,pxmit_frame->attrib.pktlen+32);
