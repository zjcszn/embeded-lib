...
u8* _rtw_malloc(const char *fun, u32 sz)
{
...
u8* _rtw_zmalloc(const char *fun, u32 sz)
{
#ifdef PLATFORM_FREEBSD
	return malloc(sz,M_DEVBUF,M_ZERO|M_NOWAIT);
#else // PLATFORM_FREEBSD

	u8 	*pbuf = _rtw_malloc(fun, sz);
...
void rtw_update_mem_stat(u8 flag, u32 sz)
{
 ...
 u8* dbg_rtw_vmalloc(u32 sz, const char *func, int line)
{
	u8  *p;
 ...
 u8* dbg_rtw_zvmalloc(u32 sz, const char *func, int line)
{
	u8 *p;
	//DBG_871X("DBG_MEM_ALLOC %s:%d %s(%d)\n", func, line, __FUNCTION__, (sz)); 
 ...
 void dbg_rtw_vmfree(u8 *pbuf, u32 sz, const char *func, int line)
{
 ...
 u8* dbg_rtw_malloc(u32 sz, const char *func, int line) 
{
	u8 *p;
 ...
 u8* dbg_rtw_zmalloc(u32 sz, const char *func, int line)
{
	u8 *p;
 ...
 void dbg_rtw_mfree(u8 *pbuf, u32 sz, const char *func, int line)
{
	if((sz)>4096)
		DBG_871X("DBG_MEM_ALLOC !!!!!!!!!!!!!! %s:%d %s(%p,%d)\n", func, line, __FUNCTION__, (pbuf), (sz));
...
void	_rtw_mutex_init(_mutex *pmutex)
{
	...
	*pmutex = mutex_init(__FUNCTION__);
#else
	init_MUTEX(pmutex);
#endif
...
void	_rtw_spinlock(_lock	*plock)
{
	...
	//spin_lock(plock);
    (unsigned int)*plock = local_irq_save();
#endif
#ifdef PLATFORM_FREEBSD
	mtx_lock(plock);
#endif
...
void	_rtw_spinunlock(_lock *plock)
{
	...
	//spin_unlock(plock);
    local_irq_restore((unsigned int)*plock);
#endif
#ifdef PLATFORM_FREEBSD
	mtx_unlock(plock);
#endif	
...
struct net_device *rtw_alloc_etherdev_with_old_priv(int sizeof_priv, void *old_priv)
{
	...
	pnetdev = (struct net_device *)mem_calloc(sizeof(struct rtw_netdev_priv_indicator) + sizeof(struct net_device), 1);
#else
