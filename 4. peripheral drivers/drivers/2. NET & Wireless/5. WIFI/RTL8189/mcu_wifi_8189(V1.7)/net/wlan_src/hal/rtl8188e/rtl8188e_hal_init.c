...
efuse_usage=efuse_usage;
	...
	{
		efuse_utilized++;
		//DBG_871X("efuse_Addr-%d efuse_data=%x\n", eFuse_Addr, *rtemp8);
		eFuse_Addr++;
	}
	else
	{
...
int rtl8188e_IOL_exec_cmds_sync(ADAPTER *adapter, struct xmit_frame *xmit_frame, u32 max_wating_ms,u32 bndy_cnt)
{
	...
	int ret = _FAIL;
	u32 t1,t2;
	passing_time_ms=passing_time_ms;
	polling_ret=polling_ret;
	t1=t1;
	t2=t2;
	//DBG_871X("===> %s ,bndy_cnt = %d \n",__FUNCTION__,bndy_cnt);
...
void rtw_IOL_cmd_tx_pkt_buf_dump(ADAPTER *Adapter,int data_len)
{
	...
	if(pbuf){
		for(addr=0;addr< data_cnts;addr++){
			//DBG_871X("==> addr:0x%02x\n",addr);
			rtw_write32(Adapter,0x140,addr);
			rtw_usleep_os(2);
			loop=0;
			do{
				rstatus=(reg_140=rtw_read32(Adapter,REG_PKTBUF_DBG_CTRL)&BIT24);	
				//DBG_871X("rstatus = %02x, reg_140:0x%08x\n",rstatus,reg_140);
				if(rstatus){
					fifo_data = rtw_read32(Adapter,REG_PKTBUF_DBG_DATA_L);
					//DBG_871X("fifo_data_144:0x%08x\n",fifo_data);					
					_rtw_memcpy(pbuf+(addr*8),&fifo_data , 4);  
					
					fifo_data = rtw_read32(Adapter,REG_PKTBUF_DBG_DATA_H);
					//DBG_871X("fifo_data_148:0x%08x\n",fifo_data);					
...
pFwImageFileName=pFwImageFileName;
pucMappedFile=pucMappedFile;
...
void rtl8188e_start_thread(_adapter *padapter)
{
	...
	/*pHalData->SdioXmitThread = */thread_create(rtl8188es_xmit_thread, padapter, TASK_WIFI5_PRIO, 0, 256, "RTWHALXT");
...
{		
	...
	for(rfPath = 0 ; rfPath < pHalData->NumTotalRFPath ; rfPath++)
	{
		...
		for(group = 0 ; group < MAX_CHNL_GROUP_24G; group++)
		{
			//DBG_871X(" IndexCCK_Base rfPath:%d group:%d,eeAddr:0x%02x ",rfPath,group,eeAddr);
			pwrInfo24G->IndexCCK_Base[rfPath][group] =	PROMContent[eeAddr++];
			//DBG_871X(" IndexCCK_Base:%02x \n",pwrInfo24G->IndexCCK_Base[rfPath][group] );
...
//				pHalData->bNOPG = TRUE; 							
		...
		{
			//DBG_871X(" IndexBW40_Base rfPath:%d group:%d,eeAddr:0x%02x ",rfPath,group,eeAddr);
			pwrInfo24G->IndexBW40_Base[rfPath][group] =	PROMContent[eeAddr++];
			//DBG_871X(" IndexBW40_Base: %02x \n",pwrInfo24G->IndexBW40_Base[rfPath][group]  );
