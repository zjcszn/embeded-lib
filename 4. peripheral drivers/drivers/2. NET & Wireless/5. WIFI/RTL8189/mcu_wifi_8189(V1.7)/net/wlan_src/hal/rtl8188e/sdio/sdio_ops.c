...
_func_enter_;
	...
	sdio_readN(pintfhdl, addr, cnt, rmem);
...
DECLARE_MONITOR_ITEM("sd recv max len", sd_recv_max_len);
...
static struct recv_buf* sd_recv_rxfifo(PADAPTER padapter, u32 size)
{
	...
	readsize = size;
#if SYS_MONITOR
	if(size > GET_MONITOR_ITEM_VALUE(sd_recv_max_len))
  		SET_MONITOR_ITEM_VALUE(sd_recv_max_len, size);
#endif
	//3 1. alloc skb
	// align to block size
	allocsize = _RND(readsize, adapter_to_dvobj(padapter)->intf_data.block_transfer_len);
...
//	rtw_read_port(padapter, WLAN_RX0FF_DEVICE_ID, readsize, preadbuf);
	...
	if (precvbuf == NULL) {
		dev_kfree_skb_any(ppkt);
		RT_TRACE(_module_hci_ops_os_c_, _drv_warning_, ("%s: alloc recvbuf FAIL!\n", __FUNCTION__));
		return NULL;
	}
...
static void sd_rxhandler(PADAPTER padapter, struct recv_buf *precvbuf)
{
	...
	schedule_tasklet(&precvpriv->recv_tasklet);
#endif
}
...
u8 RecvOnePkt(PADAPTER padapter, u32 size)
{
	...
	if(size) {
		...
		if (precvbuf) {
			//DBG_871X("Completed Recv One Pkt.\n");
			sd_rxhandler(padapter, precvbuf);
			res = _TRUE;
		}else{
			res = _FALSE;
