...
const  static  u8 sbox_table[256] =
    {
...
prwskeylen=prwskeylen;
	...
	if((pattrib->encrypt==_AES_)){
		...
		if (stainfo!=NULL){
			...
			RT_TRACE(_module_rtl871x_security_c_,_drv_warning_,("rtw_aes_encrypt: stainfo!=NULL!!!\n"));
...
_func_enter_;			
	...
	message = (u8*)mem_malloc(MAX_MSG_SIZE);
	if(!message)
		return _FAIL;
...
_func_enter_;	 
	...
	if((prxattrib->encrypt==_AES_)){
		...
		stainfo=rtw_get_stainfo(&padapter->stapriv ,&prxattrib->ta[0] );
		if (stainfo!=NULL){
			RT_TRACE(_module_rtl871x_security_c_,_drv_debug_,("rtw_aes_decrypt: stainfo!=NULL!!!\n"));
