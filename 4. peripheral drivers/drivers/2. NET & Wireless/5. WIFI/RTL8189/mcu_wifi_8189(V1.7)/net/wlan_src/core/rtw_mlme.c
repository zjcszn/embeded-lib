...
BOOL BssTabRstSimple(BSS_TABLE_SIMPLE *Tab) 
{
	uint32_t irq_flag;	
	BSS_ENTRY_SIMPLE *p_entry;
	...
	if(Tab->BssEntry)
	{
		p_entry = Tab->BssEntry;
		
		do{
			if(p_entry->used)
				p_entry->used--;
			p_dbg("dec:%s, %d", p_entry->res_data.essid, p_entry->used);
...
将used == 0的项删掉,其余的置为1
*/
BOOL BssTabFlushSimple(BSS_TABLE_SIMPLE *Tab) 
{
	uint32_t irq_flag;	
	BSS_ENTRY_SIMPLE *p_entry, *p_last/*, *p_next*/;
	...
	if(Tab->BssEntry)
	{
		do{
			...
			if(p_entry->used == 0)
			{
				p_dbg("remove old:%s", p_entry->res_data.essid);
				Tab->BssEntry = p_entry->next;
				kfree(p_entry);
			}else{
...
BOOL BssTabClrSimple(BSS_TABLE_SIMPLE *Tab) 
{
	uint32_t irq_flag;	
	BSS_ENTRY_SIMPLE *p_entry, *p_old;
	...
	if(Tab->BssEntry)
	{
		...
		do{
			p_old = p_entry;
			p_entry = p_entry->next;
			p_dbg("del:%s", p_old->res_data.essid);
			mem_free(p_old);
		}while(p_entry);
	}
...
BSS_ENTRY_SIMPLE *BssTabCloneSimple(BSS_TABLE_SIMPLE *Tab) 
{
	uint32_t irq_flag;	
	BSS_ENTRY_SIMPLE *p_entry = 0, *p_tmp = 0, *p_last = 0, *p_head = 0;
	...
	if(Tab->BssEntry)
	{
		p_entry = Tab->BssEntry;
		do{
			p_tmp = (BSS_ENTRY_SIMPLE*)mem_malloc(sizeof(BSS_ENTRY_SIMPLE));
			if(!p_tmp)
				break;
			
			memcpy(p_tmp, p_entry, sizeof(BSS_ENTRY_SIMPLE));
			if(p_last == 0)
			{
				p_head = p_last = p_tmp;
				p_tmp->next = 0;
	...
	local_irq_restore(irq_flag);
//	RTMP_IRQ_UNLOCK(Tab->lock, iflag);
	return p_head;
}

BOOL BssTabInsertSimple(
		IN BSS_TABLE_SIMPLE *Tab, BSS_ENTRY_SIMPLE *body) 
{
	int find = 0;
	uint32_t irq_flag;	
	BSS_ENTRY_SIMPLE *p_entry;
	...
	{
		p_entry = Tab->BssEntry;
		if(memcmp(p_entry->res_data.bssid, body->res_data.bssid, 6) == 0)
		{
			find = 1;
			p_dbg("find old1:%s", body->res_data.essid);
			p_entry->used++;
		}
		while(p_entry->next){
			p_entry = p_entry->next;
			...
			{
				find = 1;
				p_dbg("find old2:%s", body->res_data.essid);
				p_entry->used++;
			}
		}
		if(!find){
			p_dbg("add new:%s", body->res_data.essid);
			p_entry->next = body;
			body->next = 0;
			body->used = 3;
		}else
...
BSS_ENTRY_SIMPLE *BssTableSearchSimpleBySSID(
	IN BSS_TABLE_SIMPLE *Tab, 
	IN PUCHAR	 pssid,
		int len) 
{
	uint32_t irq_flag;	
	BSS_ENTRY_SIMPLE *p_entry = 0;
	if(!Tab->BssEntry)
		return 0;
	...
	p_entry = Tab->BssEntry;
	do{
		if((p_entry->res_data.essid_len == len) && (memcmp(p_entry->res_data.essid, pssid, len) == 0)) 
		{ 
			BSS_ENTRY_SIMPLE *p_tmp = (BSS_ENTRY_SIMPLE*)mem_malloc(sizeof(BSS_ENTRY_SIMPLE));
			if(p_tmp)
				memcpy(p_tmp, p_entry, sizeof(BSS_ENTRY_SIMPLE));
...
BSS_ENTRY_SIMPLE *BssTableSearchSimple(
	IN BSS_TABLE_SIMPLE *Tab, 
	IN PUCHAR	 pBssid) 
{
	uint32_t irq_flag;	
	...
	p_entry = Tab->BssEntry;
	do{
		if (memcmp(p_entry->res_data.bssid, pBssid, 6) == 0) 
		{ 
			local_irq_restore(irq_flag);
			return p_entry;
		}
...
void fill_scan_result_data(struct scan_result_data *p_data,
							uint8_t *bssid, 
							char *ssid, 
							int ssid_len,
							int channel,
							...
							int auth_mode)
{
	...
	memcpy(p_data->bssid, bssid, 6);
	
	p_data->channel = channel;
	memcpy(p_data->essid, ssid, 32);
	p_data->essid_len = ssid_len;
	p_data->freq = ieee80211_channel_to_frequency(channel, IEEE80211_BAND_2GHZ);
	p_data->rssi = rssi;
	p_data->mode = (bss_type == Ndis802_11IBSS)?(MODE_ADHOC):(MODE_AP);
	p_data->auth = auth_mode;
/*
	if(privacy  == 0)
	{
		p_data->proto = WPA_PROTO_NONE;
		p_data->cipher = WPA_CIPHER_NONE;
	}else{*/
...
void rtw_update_scanned_network(_adapter *adapter, WLAN_BSSID_EX *target)
{
	...
	struct wlan_network	*pnetwork = NULL;
	struct wlan_network	*oldest = NULL;
	BSS_ENTRY_SIMPLE   *BssEntry;
_func_enter_;

	BssEntry = BssTableSearchSimple((BSS_TABLE_SIMPLE*)&pmlmepriv->ScanTabSimple, target->MacAddress);
	
	if (BssEntry == 0) 
	{
		BssEntry = (BSS_ENTRY_SIMPLE*)mem_malloc(sizeof(BSS_ENTRY_SIMPLE));

		if(BssEntry)
		{
			BssEntry->next = 0;

			pnetwork = (struct wlan_network*)mem_calloc(sizeof(struct wlan_network) + 512, 1); // will update scan_time
		//	pnetwork = rtw_alloc_network(pmlmepriv); // will update scan_time
			...
			if(pnetwork){ 
				...
				fill_scan_result_data(&BssEntry->res_data,
					pnetwork->network.MacAddress,
					(char*)pnetwork->network.Ssid.Ssid,
					pnetwork->network.Ssid.SsidLength,
					adapter->dvobj->oper_channel,
					pnetwork->network.Rssi,
					pnetwork->network.InfrastructureMode,
					pnetwork->network.Privacy,
					pnetwork->BcnInfo.encryp_protocol);
								
				BssTabInsertSimple((BSS_TABLE_SIMPLE*)&pmlmepriv->ScanTabSimple, BssEntry);

				mem_free(pnetwork);
			//	rtw_free_network(pmlmepriv, pnetwork, 1);

				pnetwork = NULL;
			}else
				p_err("rtw_alloc_network err");
		}
	}
	...
	if (rtw_end_of_queue_search(phead,plist)== _TRUE) {
		...
		if (_rtw_queue_empty(&(pmlmepriv->free_bss_pool)) == _TRUE) {
            ...
            //edit by lck
			if(pmlmepriv->scan_mode == SCAN_ACTIVE &&   
                pmlmepriv->scan_ssid.SsidLength &&
			    (strncmp((char*)oldest->network.Ssid.Ssid, 
                    (char*)pmlmepriv->scan_ssid.Ssid, 
                    pmlmepriv->scan_ssid.SsidLength) == 0))
			{
                //p_err("can not del oldest");
			}else{
				pnetwork = oldest;
...
pht_addtinfo=pht_addtinfo;
	...
	len = 0;
	p = rtw_get_ie(pie+sizeof (NDIS_802_11_FIXED_IEs), _HT_CAPABILITY_IE_, (uint*)&len, ie_len-sizeof (NDIS_802_11_FIXED_IEs));
	if(p && len>0)	
	{
	...
	len=0;
	p = rtw_get_ie(pie+sizeof (NDIS_802_11_FIXED_IEs), _HT_ADD_INFO_IE_, (uint*)&len, ie_len-sizeof (NDIS_802_11_FIXED_IEs));
	if(p && len>0)	
	{
