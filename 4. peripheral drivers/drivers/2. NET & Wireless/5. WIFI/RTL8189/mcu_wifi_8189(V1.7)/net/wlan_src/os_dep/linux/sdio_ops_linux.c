...
_func_enter_;
	...
	for (i = 0; i < cnt; i++) {
		pdata[i] = sdio_readb(func->num, addr+i, &err);
		if (err) {
			DBG_871X(KERN_ERR "%s: FAIL!(%d) addr=0x%05x\n", __func__, err, addr+i);
			break;
		}
...
_func_enter_;
	...
	for (i = 0; i < cnt; i++) {
		sdio_writeb(func->num, pdata[i], addr+i, &err);
		if (err) {
...
_func_enter_;
	...
	v = sdio_readb(func->num, addr, err);
...
_func_enter_;
	...
	if (claim_needed)
		sdio_claim_host(func);
	v = sdio_readw(func->num, addr, err);
	if (claim_needed)
		sdio_release_host(func);
	if (err && *err)
		DBG_871X(KERN_ERR "%s: FAIL!(%d) addr=0x%05x\n", __func__, *err, addr);
...
_func_enter_;
	...
	v = sdio_readl(func->num, addr, err);
...
_func_enter_;
	...
	if (claim_needed)
		sdio_claim_host(func);
	sdio_writeb(func->num, v, addr, err);
	if (claim_needed)
		sdio_release_host(func);
	if (err && *err)
		DBG_871X(KERN_ERR "%s: FAIL!(%d) addr=0x%05x val=0x%02x\n", __func__, *err, addr, v);
...
_func_enter_;
	...
	if (claim_needed)
		sdio_claim_host(func);
	sdio_writew(func->num, v, addr, err);
	if (claim_needed)
		sdio_release_host(func);
	if (err && *err)
...
_func_enter_;
	...
	sdio_writel(func->num, v, addr, err);
...
_func_enter_;
	...
	if ((cnt==1) || (cnt==2))
	{
		...
		for (i = 0; i < cnt; i++)
		{
			*(pbuf+i) = sdio_readb(func->num, addr+i, &err);
	...
	err = sdio_readsb(pdata, addr, cnt);
...
_func_enter_;
	...
	func = psdio->func;
	size = /*sdio_align_size(func, cnt)*/cnt;
	...
	if ((cnt==1) || (cnt==2))
	{
		...
		for (i = 0; i < cnt; i++)
		{
			sdio_writeb(func->num, *(pbuf+i), addr+i, &err);
			if (err) {
	...
	size = cnt;
	err = sdio_writesb(addr, pdata, size);
