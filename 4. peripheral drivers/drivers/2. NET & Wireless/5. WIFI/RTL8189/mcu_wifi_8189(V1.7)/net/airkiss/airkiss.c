							...
							const airkiss_config_t* config)
{
	...
	p_dbg("air_cfg size:%d", sizeof(_airkiss_local_cfg));
	return 0;
}
							...
							const void* frame, unsigned short length)
{
	...
	//mark 一般是七十多，如果不限制一下，容易受到其他数据干扰
	//？？？？？
	if(length > 100)
		return;
	...
	if(success)
	{
		...
		resest_airkiss_data();
		p_dbg("airkiss_recv_discover success");
		p_dbg("base len:%d", air_cfg->base_len);
	}
}
							...
							const void* frame, unsigned short length)
{
			...
			(air_cfg->data.magic_code.record[3]&0xfff0)==0x0030)
	{
		...
		air_cfg->airkiss_state += 1;
		resest_airkiss_data();
		p_dbg("airkiss_process_magic_code success");
		p_dbg("total_len:%d, ssid crc:%x", air_cfg->total_len, air_cfg->ssid_crc);
	}
}
							...
							const void* frame, unsigned short length)
{
			...
			(air_cfg->data.prefix_code.record[3]&0xfff0)==0x0070)
	{
		...
		air_cfg->airkiss_state += 1;
		air_cfg->need_seq = ((air_cfg->pswd_len + 1) + 3)/4; //只取密码和random
		air_cfg->seq_success_map_cmp = (1 << air_cfg->need_seq) - 1; // EXAMPLE: need_seq = 5; seq_success_map_cmp = 0x1f;
			
		resest_airkiss_data();
		p_dbg("airkiss_process_prefix_code success");
		p_dbg("pswd_len:%d, pswd_crc:%x, need seq:%d, seq map:%x", air_cfg->pswd_len, air_cfg->pswd_crc, air_cfg->need_seq, air_cfg->seq_success_map_cmp);
	}
}
							...
							const void* frame, unsigned short length)
{
		...
		((air_cfg->data.seq_code.record[1]&0x7f) <= ((air_cfg->total_len>>2)+1)))
	{
		...
		p_dbg("seq:%d, %x,%x,%x,%x", tempBuffer[1], tempBuffer[2], tempBuffer[3], tempBuffer[4], tempBuffer[5]);
		if(tempBuffer[0] == (calcrc_bytes(tempBuffer+1,5)&0x7F) )
		{
			...
			p_dbg("now seq map:%x", air_cfg->seq_success_map);
			resest_airkiss_data();
