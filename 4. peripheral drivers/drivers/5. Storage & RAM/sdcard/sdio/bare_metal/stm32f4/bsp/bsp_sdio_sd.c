...
static volatile uint8_t sd_dmarx_busy = 0;
static volatile uint8_t sd_dmatx_busy = 0;
...
{  
  /* Check DMA rx transfer busy status */
...
{ 
  /* Check DMA tx transfer busy status */
...
/**
  * @brief  This function waits until the SDIO DMA RX data transfer is finished.
  * @retval SD status
  */
uint8_t BSP_SD_WaitReadOperation(void)
{
  uint32_t timeout = SD_DATATIMEOUT;

  while(sd_dmarx_busy != 0)
  {
    if (timeout-- == 0) return MSD_ERROR;
  }
  return MSD_OK;
}
...
/**
  * @brief  This function waits until the SDIO DMA TX data transfer is finished.
  * @retval SD status
  */
uint8_t BSP_SD_WaitWriteOperation(void)
{
  uint32_t timeout = SD_DATATIMEOUT;

  while(sd_dmatx_busy != 0)
  {
    if (timeout-- == 0) return MSD_ERROR;
  }
  return MSD_OK;
