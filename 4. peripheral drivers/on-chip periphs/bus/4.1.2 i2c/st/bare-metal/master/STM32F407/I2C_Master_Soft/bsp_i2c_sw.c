...
#include "stm32h7xx_ll_bus.h"
#include "elog.h"
#define   SOFT_I2C_LOG(...)         log_d(__VA_ARGS__)
#else
...
static inline void I2C_DELAY_EX(soft_i2c_dev_t *i2c_dev) {
    SOFT_I2C_DELAY(i2c_dev->ck_delay + 1);
}
...
static void soft_i2c_master_restart(soft_i2c_dev_t *i2c_dev);
 ...
 */
static void soft_i2c_gpio_clk_enable(GPIO_TypeDef *gpio_x) {
    if (gpio_x == GPIOA) LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOA);
    else if (gpio_x == GPIOB) LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOB);
    else if (gpio_x == GPIOC) LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOC);
    else if (gpio_x == GPIOD) LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOD);
    else if (gpio_x == GPIOE) LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOE);
    else if (gpio_x == GPIOF) LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOF);
    else if (gpio_x == GPIOG) LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOG);
    else if (gpio_x == GPIOH) LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOH);
    else if (gpio_x == GPIOI) LL_AHB4_GRP1_EnableClock(LL_AHB4_GRP1_PERIPH_GPIOI);
}
...
void soft_i2c_init(soft_i2c_dev_t *i2c_dev, uint16_t i2c_freq) {
    ...
    i2c_dev->ck_delay = ck_delay ? ck_delay : 2;                  // calculate i2c clock delay
...
void soft_i2c_master_bus_reset(soft_i2c_dev_t *i2c_dev) {
    soft_i2c_master_restart(i2c_dev);
 ...
 * @return int 0 on success | others on failed
 */
int soft_i2c_master_mem_write(soft_i2c_dev_t *i2c_dev,
                              uint32_t mem_addr,
                              uint16_t addr_len,
 ...
 * @return int  0 on success | others on failed
 */
int soft_i2c_master_mem_read(soft_i2c_dev_t *i2c_dev,
                             uint32_t mem_addr,
                             uint16_t addr_len,
                             uint8_t *rd_data,
                             uint16_t data_len) {
    ...
    // fixed by zjcszn, 250802: PCA9535 不支持Repeat Start
    soft_i2c_master_restart(i2c_dev);
    
    // send slave address (read mode)
    uint8_t rd_addr = ((i2c_dev->dev_addr << 1) & 0xFE) | 0x01;
    if (soft_i2c_master_send_byte(i2c_dev, rd_addr)) {
        SOFT_I2C_LOG("Soft I2C ERROR -> RD Addr No Response: 0x%02X\r\n", i2c_dev->dev_addr);
        return SOFT_I2C_ERR_ADDR;
    }
 ...
 */
static int soft_i2c_master_send_byte(soft_i2c_dev_t *i2c_dev, volatile uint8_t byte) {
    for (int i = 0; i < 8; i++) {
        I2C_SCL_L(i2c_dev);
        byte & 0x80 ? I2C_SDA_H(i2c_dev) : I2C_SDA_L(i2c_dev);
        I2C_DELAY(i2c_dev);
        I2C_SCL_H(i2c_dev);
        I2C_DELAY_EX(i2c_dev);
        byte <<= 1U;
    }
