...
#define SPI1_TX_DMA_STREAM_ID         3                    // TX DMA数据流ID
#define SPI1_TX_DMA_STREAM            LL_DMA_STREAM_3      // TX DMA数据流
#define SPI1_TX_DMA_CHANNAL           LL_DMA_CHANNEL_3     // TX DMA通道
#define SPI1_TX_DMA_IRQN              DMA2_Stream3_IRQn    // TX DMA数据流中断号
...
typedef struct {
  __IO uint8_t   mode;          // 传输模式：WR & RD | WR & STOP | WR & WR
  __IO uint8_t   lock;          // SPI锁定标志
  __IO uint8_t   state;         // SPI传输状态标志
  __IO uint16_t  txbuf_size;    // SPI发送缓冲区大小
  __IO uint16_t  rxbuf_size;    // SPI接收缓冲区大小（可复用为次发送缓冲区大小）
