...
CO_error(CO_EM_t* em, bool_t setError, const uint8_t errorBit, uint16_t errorCode, uint32_t infoCode) {
    ...
    /* output error report */
#if (CO_EMERGENCY_LOG_ENABLE == 1)
    if (setError) {
        CO_LOG_Error("CO_EM: [SET] errorCode=0x%04X, index=%d, errorBit=0x%02X, infoCode=0x%08X",
                      errorCode, index, errorBit, infoCode);
    } else {
        CO_LOG_Info("CO_EM: [RESET] errorCode=0x%04X, index=%d, errorBit=0x%02X, infoCode=0x%08X",
                      errorCode, index, errorBit, infoCode);
    }
#endif
