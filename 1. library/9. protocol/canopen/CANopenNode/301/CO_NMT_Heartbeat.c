...
CO_NMT_process(CO_NMT_t* NMT, CO_NMT_internalState_t* NMTstate, uint32_t timeDifference_us, uint32_t* timerNext_us) {
    ...
    if (NMT->internalCommand != CO_NMT_NO_COMMAND) {
        ...
        log_d("NMT Command: %s", (NMT->internalCommand == CO_NMT_ENTER_OPERATIONAL) ? "Enter Operational" : 
                                 (NMT->internalCommand == CO_NMT_ENTER_STOPPED) ? "Enter Stopped" :
                                 (NMT->internalCommand == CO_NMT_ENTER_PRE_OPERATIONAL) ? "Pre-Operational" :
                                 (NMT->internalCommand == CO_NMT_RESET_NODE) ? "Reset Node" :
                                 (NMT->internalCommand == CO_NMT_RESET_COMMUNICATION) ? "Reset Communication" : "Error");
        #endif
