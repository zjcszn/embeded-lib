...
STATIC eMBErrorCode
eMBPUDPClientNewDataCB( xMBHandle xMBHdll );
...
eMBMUDPFrameInit( xMBMInternalHandle *pxIntHdl, const CHAR *pcUDPBindAddress, LONG uUDPListenPort )
{
    ...
    if( NULL != pxIntHdl )
    {
        ...
        if( NULL != pxFrameHdl )
        {
            if( MB_ENOERR == eMBPUDPClientInit( &( pxFrameHdl->xUDPHdl ), pxIntHdl, pcUDPBindAddress, uUDPListenPort,
    ...
    if( bMBMIsHdlValid( pxIntHdl ) )
    {
            ...
            ( usMBPDULength <= ( MBM_UDP_PDU_SIZE_MAX - 7 /* MBAP Header */ ) ) )
        {
            if( NULL != pxFrameHdl->pcUDPClientAddress )
            {
                ...
                eStatus2 = eMBPUDPConWrite( pxFrameHdl->xUDPHdl, pxFrameHdl->pcUDPClientAddress,
                                            pxFrameHdl->usClientPort, pxFrameHdl->arubBuffer, usPayloadLength );
                    ...
                    case MB_ENOERR:
                        ...
                        if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_UDP ) )
                        {
                            ...
                            vMBPPortLog( MB_LOG_DEBUG, MB_LOG_UDP,
                                         ...
                                         "] Sending new frame with MODBUS TCP PDU length=" MBP_FORMAT_USHORT ".\n",
                        ...
                        ( MBM_UDP_PROTOCOL_ID == usMBAPProtocolID ) )
                    {
                        *pusMBPDULength = ( USHORT )( pxFrameHdl->usBufferPos - MBM_UDP_MB_PDU_OFF );
                        ...
                        if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_UDP ) )
                        {
                            ...
                            vMBPPortLog( MB_LOG_DEBUG, MB_LOG_UDP,
                                         "[IDX=" MBP_FORMAT_USHORT
                                         "] Received correct frame with length=" MBP_FORMAT_USHORT ".\n",
...
#if defined( MBM_ENABLE_DEBUG_FACILITY_HEAVY ) && ( MBM_ENABLE_DEBUG_FACILITY_HEAVY == 1 )
                            vMBMLogUDPFrame(
                                MB_LOG_DEBUG, pxIntHdl, "Received frame with incorrect transaction id or protocol: ",
                                ( const UBYTE * )&pxFrameHdl->arubBuffer[MBM_UDP_TID_OFF], pxFrameHdl->usBufferPos );
#else
                            vMBPPortLog( MB_LOG_DEBUG, MB_LOG_UDP,
                                         "[IDX=" MBP_FORMAT_USHORT
