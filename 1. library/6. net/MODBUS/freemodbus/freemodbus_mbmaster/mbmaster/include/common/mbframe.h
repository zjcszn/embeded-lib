 ...
 * \return \c TRUE if this function code is an exception. Otherwise \c FALSE.
 */
#define MB_PDU_FUNC_ISEXCEPTION_FOR( ubFuncByte, ubFuncExpected ) ( ( ubFuncByte ) == ( ( ubFuncExpected ) + 0x80 ) )
 ...
 *   address.
 */
#define MB_IS_VALID_WRITE_ADDR( ucAddr ) ( ( ( ucAddr ) == MB_SER_BROADCAST_ADDR ) || MB_IS_VALID_READ_ADDR( ucAddr ) )
...
#define MB_IS_VALID_TCP_ADDR( ucAddr )                                                          \
    ( ( ( ( ucAddr ) >= MB_SER_SLAVE_ADDR_MIN ) && ( ( ucAddr ) <= MB_SER_SLAVE_ADDR_MAX ) ) || \
      ( MB_ANY_ADDR == ( ucAddr ) ) || ( MB_SER_BROADCAST_ADDR == ( ucAddr ) ) )
