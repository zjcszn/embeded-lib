        ...
        switch( *peState )
        {
            ...
            case MBM_STATE_NONE:
                if( ( ( ( ULONG )usRegStartAddress + ( ULONG )ubNRegs ) < ( ULONG )0x10000 ) && ( ubNRegs > 0 ) &&
                    ( ubNRegs <= MBM_PDU_FUNC_RD_REGCNT_MAX ) )
                {
                    ...
                    if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
                    {
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                     ...
                                     "] Read input registers request (start=" MBP_FORMAT_USHORT
...
#if MBM_FUNC_RD_INPUT_REGS_ENABLED == 1
eMBErrorCode
eMBMReadInputRegisters( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT usRegStartAddress, UBYTE ubNRegs,
