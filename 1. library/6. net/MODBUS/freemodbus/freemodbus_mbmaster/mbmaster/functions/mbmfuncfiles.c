                          ...
                          eMBErrorCode *peStatus )
{
...
#endif
    {
        switch( *peState )
        {
            ...
            case MBM_STATE_NONE:
                if( ( NULL != arxSubRequests ) && ( NULL != arxSubResponses ) && ( usNSubRequests > 0 ) &&
                    ( usNSubRequests <= MBM_PDU_FUNC_RD_FILE_MAX_SUBREQUESTS ) )
                {
                    ...
                    pxIntHdl->usFrameMBPDULength++;
                    for( usExpectedResponseLength = MBM_PDU_FUNC_RD_FILE_RESPSIZE_MIN, bSubRequestsValid = TRUE,
                         ubIdx = 0;
                         ubIdx < usNSubRequests; ubIdx++ )
                    {
                        ...
                        else
                        {
                            ...
                            usExpectedResponseLength += MBM_PDU_FUNC_RD_FILE_RESPSIZE_SUBRESPONSE_LENGTH_MIN +
                           ...
                           USHORT usNSubRequests, eMBMQueryState *peState, eMBErrorCode *peStatus )
{
...
#endif
    {
        switch( *peState )
        {
                /* In this state we prepare the frame and send it. */
            case MBM_STATE_NONE:
                if( ( NULL != arxSubRequests ) && ( usNSubRequests > 0 ) &&
                    ( usNSubRequests <= MBM_PDU_FUNC_WR_FILE_MAX_SUBREQUESTS ) )
                {
                    pxIntHdl->usFrameMBPDULength = 0;
                    ...
                    for( usExpectedResponseLength = MBM_PDU_FUNC_WR_FILE_REQ_SIZE_MIN, bSubRequestsValid = TRUE,
                         ubIdx = 0;
                         ubIdx < usNSubRequests; ubIdx++ )
                    {
                        usExpectedResponseLength +=
                            MBM_PDU_FUNC_WR_FILE_SUBREQ_SIZE_MIN + 2 * arxSubRequests[ubIdx].usRecordLength;
                        ...
                        else
                        {
                            ...
                            for( ubNBytesWritten = 0; ubNBytesWritten < 2 * arxSubRequests[ubIdx].usRecordLength;
                                 ubNBytesWritten += 2 )
                            {
                                pxIntHdl->pubFrameMBPDUBuffer[pxIntHdl->usFrameMBPDULength] =
                                    arxSubRequests[ubIdx].pubRecordData[ubNBytesWritten];
                                pxIntHdl->usFrameMBPDULength++;
                                pxIntHdl->pubFrameMBPDUBuffer[pxIntHdl->usFrameMBPDULength] =
                                    arxSubRequests[ubIdx].pubRecordData[ubNBytesWritten + 1];
                                pxIntHdl->usFrameMBPDULength++;
                            }
                        }
                    }
                    if( bSubRequestsValid && usExpectedResponseLength < MB_PDU_SIZE_MAX )
                    {
                        pxIntHdl->pubFrameMBPDUBuffer[MBM_PDU_FUNC_RD_FILE_RESPSIZE_DATA_LENGTH_OFF] =
                            usExpectedResponseLength - MBM_PDU_FUNC_WR_FILE_REQ_SIZE_MIN;
                        *peState = MBM_STATE_SEND;
                    }
                    else
                    {
                    ...
                    ( MBM_FUNC_WR_FILE_RECORD == pxIntHdl->pubFrameMBPDUBuffer[MB_PDU_FUNC_OFF] ) )
                {
                    ...
                    if( ubNBytesLeft == ( pxIntHdl->usFrameMBPDULength - MBM_PDU_FUNC_WR_FILE_REQ_SIZE_MIN ) )
                    {
                        ...
                        for( bSubRequestsValid = TRUE, ubIdx = 0; ubIdx < usNSubRequests; ubIdx++ )
                        {
                            ...
                            {
                                ubRefType = *pubCurPtr;
				(void)ubRefType;	// it is not used in current implementation
                                pubCurPtr++;
                                    ...
                                    ( usRecordLength != arxSubRequests[ubIdx].usRecordLength ) )
                                {
                                    ...
                                    break;
                                         ...
                                         memcmp( arxSubRequests[ubIdx].pubRecordData, pubCurPtr, 2 * usRecordLength ) )
                                {
                                    bSubRequestsValid = FALSE;
                                    break;
                                }
