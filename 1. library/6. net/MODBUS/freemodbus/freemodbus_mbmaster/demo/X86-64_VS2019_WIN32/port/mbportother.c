...
Error2String( DWORD dwError )
{
    ...
    dwLength = FormatMessageA( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwError,
...
vMBPPortLog( eMBPortLogLevel eLevel, eMBPortLogFacility eModule, const CHAR *szFmt, ... )
{
    ...
    if( bMBPPortLogIsEnabled( eLevel, eModule ) )
    {
        ...
        j = sprintf_s( &szBuf[i], sizeof( szBuf ) / sizeof( szBuf[0] ) - i, "%04u-%02u-%02uT%02u:%02u:%02u:%03uZ;",
                       xSystemTime.wYear, xSystemTime.wMonth, xSystemTime.wDay, xSystemTime.wHour, xSystemTime.wMinute,
                       xSystemTime.wSecond, xSystemTime.wMilliseconds );
