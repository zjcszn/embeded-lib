...
eMBPSerialInit( xMBPSerialHandle *pxSerialHdl, UCHAR ucPort, ULONG ulBaudRate, UCHAR ucDataBits,
                eMBSerialParity eParity, UCHAR ucStopBits, xMBHandle xMBMHdl, pbMBPTimerExpiredCB pbFrameTimeoutFN,
                eMBSerialMode eMode )
{
    ...
    if( NULL != pxSerHdl )
    {
        ...
        if( INVALID_HANDLE_VALUE == ( pxSerHdl->xSerHdl = CreateFileA( szDevice, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                        ...
                        switch( pxSerHdl->eMode )
                        {
                            ...
                            case MB_RTU:
                                if( 0x0000U == ( usCRC16 = prvCRC16Update( usCRC16, arubBuffer[dwBytesReadTotal] ) ) )
                                {
                                    ...
                                    {
                                        if( bMBGuessRTUFrameIsComplete( arubBuffer,
                                                                        ( USHORT )( dwBytesReadTotal + 1 ) ) )
                                        {
                                            bFrameIsComplete = TRUE;
                                        }
                                    }
