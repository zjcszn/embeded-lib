...
STATIC void *
vMBUDPClientHandlerThread( void *pxUDPIntHdl );
                   ...
                   peMBPUDPClientNewDataCB eMBPUDPClientNewDATAFN )
{
    ...
    if( NULL != pxUDPHdl )
    {
        ...
        if( NULL != pxUDPIntHdl )
        {
            ...
            else if( SOCKET_ERROR == bind( pxUDPIntHdl->xUDPListeningSocket.fd,
                                           ( SOCKADDR * )&pxUDPIntHdl->xUDPListeningAddr,
            ...
            else if( SOCKET_ERROR == getsockname( pxUDPIntHdl->xUDPListeningSocket.fd,
                                                  ( SOCKADDR * )&pxUDPIntHdl->xUDPListeningAddr, &iSockLen ) )
            {
#if defined( MBP_ENABLE_DEBUG_FACILITY ) && ( MBP_ENABLE_DEBUG_FACILITY == 1 )
                if( bMBPPortLogIsEnabled( MB_LOG_ERROR, MB_LOG_PORT_UDP ) )
                {
            ...
            else if( SOCKET_ERROR == WSAEventSelect( pxUDPIntHdl->xUDPListeningSocket.fd,
                                                     pxUDPIntHdl->xUDPListeningSocket.xWSAEvent, FD_READ ) )
            {
            }
            else
            {
                ...
                if( bMBPPortLogIsEnabled( MB_LOG_INFO, MB_LOG_PORT_UDP ) )
                {
                    ...
                    if( SOCKET_ERROR == WSAAddressToStringA( ( LPSOCKADDR )&pxUDPIntHdl->xUDPListeningAddr,
                                                             sizeof( pxUDPIntHdl->xUDPListeningAddr ), NULL,
                                                             arcNameBuffer, &dwNameBufferLen ) )
                    {
...
eMBPUDPInitCommon( xMBPUDPIntHandle *pxUDPIntHdl, void *( *pvHandlerThread )( void * ))
{
    ...
    if( NULL == ( pxUDPIntHdl->pxHandlingThread = CreateThread(
                      NULL, THREAD_STACKSIZE_MAX, ( LPTHREAD_START_ROUTINE )pvHandlerThread, pxUDPIntHdl, 0, NULL ) ) )
    {
#if defined( MBP_ENABLE_DEBUG_FACILITY ) && ( MBP_ENABLE_DEBUG_FACILITY == 1 )
        if( bMBPPortLogIsEnabled( MB_LOG_ERROR, MB_LOG_PORT_UDP ) )
        {
    ...
    do
    {
        ...
        if( INVALID_SOCKET != pxUDPIntHdl->xUDPListeningSocket.fd )
        {
            ...
            else if( iResult > 0 )
            {
                ...
                if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_PORT_UDP ) )
                {
                    vMBPPortLog( MB_LOG_DEBUG, MB_LOG_PORT_TCP, "[CL=%hu] New data for client (socket=%d).\n",
                 ...
                 USHORT usBufferLen )
{
    ...
    if( NULL != pxUDPIntHdl )
    {
        ...
        {
            dwNameBufferLen = _countof( arcNameBuffer );
            if( SOCKET_ERROR == WSAAddressToStringA( ( LPSOCKADDR )&xClientAddr, sizeof( xClientAddr ), NULL,
                                                     arcNameBuffer, &dwNameBufferLen ) )
            {
                strcpy( arcNameBuffer, "unknown" );
            }
            if( !sendto( pxUDPIntHdl->xUDPListeningSocket.fd, pubBuffer, usBufferLen, 0, ( SOCKADDR * )&xClientAddr,
                         sizeof( xClientAddr ) ) )
            {
#if defined( MBP_ENABLE_DEBUG_FACILITY ) && ( MBP_ENABLE_DEBUG_FACILITY == 1 ) && ( MBP_UDP_DEBUG == 1 )
                if( bMBPPortLogIsEnabled( MB_LOG_WARN, MB_LOG_PORT_UDP ) )
                   ...
                   USHORT *pusBufferLen, USHORT usBufferMax )
{
    ...
    if( NULL != pxUDPIntHdl )
    {
        iBytesReceived = recvfrom( pxUDPIntHdl->xUDPListeningSocket.fd, pubBuffer, usBufferMax, 0,
                                   ( LPSOCKADDR )&xSockAddr, &xSockAddrLen );
        ...
        else
        {
            ...
            if( 0 != WSAAddressToStringA( ( LPSOCKADDR )&xSockAddr, sizeof( xSockAddr ), NULL, arcNameBuffer,
                                          &dwNameBufferLen ) )
            {
#if defined( MBP_ENABLE_DEBUG_FACILITY ) && ( MBP_ENABLE_DEBUG_FACILITY == 1 ) && ( MBP_UDP_DEBUG == 1 )
                if( bMBPPortLogIsEnabled( MB_LOG_ERROR, MB_LOG_PORT_UDP ) )
