...
STATIC void
vMBTCPHandleReset( xMBPTCPIntHandle *pxTCPHdl, BOOL bClose );
STATIC void *
vMBTCPClientHandlerThread( void *xMBPTCPIntHandle );
STATIC void
vMBTCPClientHandleReset( struct pollfd *pxPollFd, BOOL bClose );
STATIC void *
vMBTCPServerHandlerThread( void *xMBPTCPIntHandle );
STATIC void
vMBPTCPSocketEnableKeepalive( int socket, int iKeepAlive, int iKeepAliveTime, int iIntvl, int iProbes );

/* ----------------------- Start implementation -----------------------------*/
eMBErrorCode
eMBPTCPClientInit( xMBPTCPHandle *pxTCPHdl, xMBHandle xMBHdlArg, peMBPTCPClientNewDataCB eMBPTCPClientNewDATAFNArg,
        ...
        if( NULL != pxPollFd )
        {
            ...
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_PORT_TCP ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_PORT_TCP,
                             "[IDX %d]: marked client connection as closed (socket=%d).\n", pxTCPIntHdl->ubIdx,
                             pxPollFd->fd );
            }
#endif
            ...
            if( iRes != 0 )
            {
                ...
                else if( iRes > 0 )
                {
                    if( 0 != ( pxTCPIntHdl->xShared.xClientSock.xClientSocket.revents & POLLHUP ) )
                    {
                        ...
                        if( bMBPPortLogIsEnabled( MB_LOG_INFO, MB_LOG_PORT_TCP ) )
                        {
                            vMBPPortLog( MB_LOG_INFO, MB_LOG_PORT_TCP, "[CL %d]: client (fd=%d) disconnected.\n",
                                         pxTCPIntHdl->ubIdx, pxTCPIntHdl->xShared.xClientSock.xClientSocket.fd );
                        }
#endif
                        if( NULL != pxTCPIntHdl->xShared.xClientSock.eMBPTCPClientDisconnectedFN )
                        {
                            pxTCPIntHdl->xShared.xClientSock.eMBPTCPClientDisconnectedFN(
                                pxTCPIntHdl->xMBHdl, &( pxTCPIntHdl->xShared.xClientSock.xClientSocket ) );
                        }
                    }
                    else if( 0 != ( pxTCPIntHdl->xShared.xClientSock.xClientSocket.revents & POLLIN ) )
                    {
                        ...
                        {
                            pxTCPIntHdl->xShared.xClientSock.eMBPTCPClientNewDataFN(
                                pxTCPIntHdl->xMBHdl, &( pxTCPIntHdl->xShared.xClientSock.xClientSocket ) );
                        }
                    }
                }
...
eMBErrorCode
eMBPTCPServerInit( xMBPTCPHandle *pxTCPHdl, CHAR *pcBindAddress, USHORT usTCPPort, xMBHandle xMBHdlArg,
                    ...
                    else
                    {
                        ...
                        if( bMBPPortLogIsEnabled( MB_LOG_ERROR, MB_LOG_PORT_TCP ) )
                        {
                            vMBPPortLog( MB_LOG_ERROR, MB_LOG_PORT_TCP,
                                         "[SRV %d] resetting handle, closing socket: %s\n", iSockAddr,
                                         strerror( errno ) );
                        }
#endif
                        vMBTCPHandleReset( pxTCPIntHdl, TRUE );
...
vprvMBTCPServerAcceptClient( xMBPTCPIntHandle *pxTCPIntHdl, int xListeningSocket )
{
    ...
    else if( -1 == fcntl( xNewClientSocket, F_SETFL, O_NONBLOCK ) )
    {
        ...
        {
            vMBPPortLog( MB_LOG_ERROR, MB_LOG_PORT_TCP,
                         "[SRV %d] dropped client connection because of lack of resources!", pxTCPIntHdl->ubIdx );
        }
#endif
    ...
    else
    {
        ...
        for( ubClientSockIdx = 0; ubClientSockIdx < MAX_CLIENT_SOCKS; ubClientSockIdx++ )
        {
            ...
            if( INVALID_SOCKET == pxTCPClientIntHdl->fd )
            {
                ...
                vMBPTCPSocketEnableKeepalive( pxTCPClientIntHdl->fd, TCP_KEEPALIVE, TCP_KEEPALIVE_TIME,
                                              TCP_KEEPALIVE_INTVL, TCP_KEEPALIVE_PROBES );
                MBP_ASSERT( NULL != pxTCPServerIntHdl->eMBPTCPClientConnectedFN );
                bDropClient = TRUE;
                if( pxTCPIntHdl->bIsRunning && ( MB_ENOERR == pxTCPServerIntHdl->eMBPTCPClientConnectedFN(
                                                                  pxTCPIntHdl->xMBHdl, pxTCPClientIntHdl ) ) )
                {
#if defined( MBP_ENABLE_DEBUG_FACILITY ) && ( MBP_ENABLE_DEBUG_FACILITY == 1 )
                    if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_PORT_TCP ) )
                    {
                ...
                if( bDropClient )
                {
                    ...
                    {
                        vMBPPortLog( MB_LOG_ERROR, MB_LOG_PORT_TCP,
                                     "[SRV %d] instance closed or lacking resources. droping client!\n",
                                     pxTCPIntHdl->ubIdx );
                    }
#endif
            ...
            for( x = 0; x < MAX_CLIENT_SOCKS; x++ )
            {
                ...
                if( 0 != ( pxTCPServerIntHdl->xClientSocket[x].revents & POLLIN ) )
                {
                    ...
                    ( void )pxTCPServerIntHdl->eMBPTCPClientNewDataFN( pxTCPIntHdl->xMBHdl,
                                                                       &( pxTCPServerIntHdl->xClientSocket[x] ) );
                }
                else if( 0 != ( pxTCPServerIntHdl->xClientSocket[x].revents ) )
                {
                    ...
                    {
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_PORT_TCP,
                                     "[SRV %d] unexpected poll events for client %d (events=%d).\n", pxTCPIntHdl->ubIdx,
                                     x, pxTCPServerIntHdl->xClientSocket[x].revents );
                    }
#endif
                    bDropClient = TRUE;
                }
                ...
                if( bDropClient )
                {
                    ...
                    {
                        MBP_EXIT_CRITICAL_SECTION();
                        ( void )pxTCPServerIntHdl->eMBPTCPClientDisconnectedFN(
                            pxTCPIntHdl->xMBHdl, &( pxTCPServerIntHdl->xClientSocket[x] ) );
                        MBP_ENTER_CRITICAL_SECTION();
                ...
                else if( 0 != pxTCPServerIntHdl->xSrvSocket[x].revents )
                {
                    ...
                    if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_PORT_TCP ) )
                    {
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_PORT_TCP, "[SRV %d] unexpected poll events %d.\n",
                                     pxTCPIntHdl->ubIdx, pxTCPServerIntHdl->xSrvSocket[x].revents );
                    }
#endif
                    bDropClient = TRUE;
                    pxTCPIntHdl->bIsRunning = FALSE;
            ...
            *pusBufferLen = 0;
            ...
            if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_PORT_TCP ) )
            {
                vMBPPortLog( MB_LOG_DEBUG, MB_LOG_PORT_TCP, "[IDX %d]: read returned %d bytes (fd=%d): %s\n",
                             pxTCPIntHdl->ubIdx, iBytesRead, pxPollFd->fd, strerror( errno ) );
            }
#endif
    ...
    if( NULL != pxPollFd )
    {
        ...
        if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_PORT_TCP ) )
        {
            vMBPPortLog( MB_LOG_DEBUG, MB_LOG_PORT_TCP, "[IDX %d]: wrote %d bytes of %d bytes (fd=%d). Status=%d\n",
                         pxTCPIntHdl->ubIdx, ( int )( ( ssize_t )usBufferLen - iBytesLeft ), ( int )usBufferLen,
                         pxPollFd->fd, eStatus );
        }
#endif
    }
