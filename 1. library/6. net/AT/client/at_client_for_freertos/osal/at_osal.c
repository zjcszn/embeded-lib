 ...
 * 
 * @return   Return the operation status. ONLY When the return value is AT_EOK, the operation is successful.
 *           If the return value is any other values, it means that the mutex take failed.
 */
int rtos_mutex_take(rtos_mutex_t mutex, uint32_t timeout, uint32_t call_flag)
{
  (void)call_flag;
  return (xSemaphoreTake(mutex, pdMS_TO_TICKS(timeout)) == pdPASS) ? AT_EOK : -AT_ETIMEOUT;
}
 ...
 * @param    call_flag is a caller flag. value can be RTOS_FROM_THREAD | RTOS_FROM_ISR.
 * 
 * @return   Return the operation status. When the return value is AT_EOK, the operation is successful.
 *           If the return value is any other values, it means that the mutex release failed.
 */
int rtos_mutex_give(rtos_mutex_t mutex, uint32_t call_flag)
{
  (void)call_flag;
  return (xSemaphoreGive(mutex) == pdPASS) ? AT_EOK : -AT_ETIMEOUT;
}
 ...
 */
rtos_sem_t rtos_sem_create(uint32_t value)
{
  if (value) 
  {
 ...
 */
int rtos_sem_take(rtos_sem_t sem, uint32_t timeout, uint32_t call_flag)
{
  (void) call_flag;
  return (xSemaphoreTake((SemaphoreHandle_t)sem, pdMS_TO_TICKS(timeout)) == pdPASS) ? AT_EOK : -AT_ETIMEOUT;
}
 ...
 */
int rtos_sem_give(rtos_sem_t sem, uint32_t call_call_flag)
{
  if (call_call_flag == RTOS_FROM_THREAD) 
  {
    return (xSemaphoreGive((SemaphoreHandle_t)sem) == pdPASS) ? AT_EOK : -AT_ETIMEOUT;
  }
  else 
  {
    ...
    return (ret == pdPASS) ? AT_EOK : -AT_ETIMEOUT;
  }
}
 ...
 */
uint32_t rtos_tick_from_ms(uint32_t ms)
{
