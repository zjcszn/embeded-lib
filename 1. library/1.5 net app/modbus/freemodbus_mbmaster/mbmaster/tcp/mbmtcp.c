...
eMBMTCPFrameInit( xMBMInternalHandle *pxIntHdl )
{
    ...
    if( NULL != pxIntHdl )
    {
        ...
        if( NULL != pxFrameHdl )
        {
            if( MB_ENOERR == eMBPTCPClientInit( &( pxFrameHdl->xTCPHdl ), pxIntHdl, eMBPTCPClientNewDataCB,
    ...
    if( bMBMIsHdlValid( pxIntHdl ) )
    {
            ...
            ( usMBPDULength <= ( MBM_TCP_PDU_SIZE_MAX - 7 /* MBAP Header */ ) ) )
        {
            if( MBP_TCPHDL_CLIENT_INVALID != pxFrameHdl->xTCPClientHdl )
            {
                ...
                do
                {
                    eStatus2 = eMBPTCPConRead( pxFrameHdl->xTCPHdl, pxFrameHdl->xTCPClientHdl, arubTempBuffer,
                                               &usPayloadLength, MB_UTILS_NARRSIZE( arubTempBuffer ) );
                ...
                eStatus2 = eMBPTCPConWrite( pxFrameHdl->xTCPHdl, pxFrameHdl->xTCPClientHdl, pxFrameHdl->arubBuffer,
                                            usPayloadLength );
                    ...
                    *pusMBPDULength = ( USHORT )( pxFrameHdl->usBufferPos - MBM_TCP_MB_PDU_OFF );
                    ...
                    if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_TCP ) )
                    {
                        ...
                        vMBMLogTCPFrame( MB_LOG_DEBUG, pxIntHdl,
                                         "Received frame: ", ( const UBYTE * )&pxFrameHdl->arubBuffer[MBM_TCP_TID_OFF],
...
#if defined( MBM_ENABLE_DEBUG_FACILITY_HEAVY ) && ( MBM_ENABLE_DEBUG_FACILITY_HEAVY == 1 )
                        vMBMLogTCPFrame(
                            MB_LOG_DEBUG, pxIntHdl, "Received frame with incorrect length or wrong slave address: ",
                            ( const UBYTE * )&pxFrameHdl->arubBuffer[MBM_TCP_TID_OFF], pxFrameHdl->usBufferPos );
#else
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_TCP,
                                     "[IDX=" MBP_FORMAT_USHORT
                                     "] Received frame with incorrect length " MBP_FORMAT_USHORT
                                     "or wrong slave address!\n",
                                     ( USHORT )pxFrameHdl->ubIdx, pxFrameHdl->usBufferPos );
#endif
                    }
...
eMBPTCPClientNewDataCB( xMBHandle xMBHdl, xMBPTCPClientHandle xTCPClientHdl )
{
    ...
    if( bMBMIsHdlValid( pxIntHdl ) && MB_IS_VALID_HDL( ( xMBMTCPFrameHandle * )pxIntHdl->xFrameHdl, xMBMTCPFrameHdl ) )
    {
        ...
        if( pxFrameHdl->eState != STATE_RCV )
        {
            ...
            if( MB_ENOERR != eMBPTCPConRead( pxFrameHdl->xTCPHdl, pxFrameHdl->xTCPClientHdl, arubBuffer, &usBytesRead,
                                             MB_UTILS_NARRSIZE( arubBuffer ) ) )
            {
                bClientBroken = TRUE;
            }
        }
        ...
        else
        {
            ...
            {
                eStatus2 = eMBPTCPConRead( pxFrameHdl->xTCPHdl, pxFrameHdl->xTCPClientHdl,
                ...
                switch( eStatus2 )
                {
                    case MB_ENOERR:
                        ...
                        if( ( USHORT )( usMBAPLengthField + MBM_TCP_UID_OFF ) == pxFrameHdl->usBufferPos )
                        {
                            ...
                            {
                                if( MB_ENOERR !=
                                    eMBPEventPost( pxIntHdl->xFrameEventHdl, ( xMBPEventType )MBM_EV_RECEIVED ) )
                                {
                                    eStatus = MB_EPORTERR;
                                }
                            }
                            else
                            {
                                ...
                                if( bMBPPortLogIsEnabled( MB_LOG_WARN, MB_LOG_TCP ) )
                                {
                                    vMBPPortLog( MB_LOG_WARN, MB_LOG_TCP,
                                                 "[IDX=" MBP_FORMAT_USHORT
                                                 "] received transaction ID " MBP_FORMAT_USHORT
                                                 " does not match " MBP_FORMAT_USHORT "!\n",
                                                 ( USHORT )pxFrameHdl->ubIdx, usMBAPTransactionIDField,
                                                 pxFrameHdl->usCurrentTID );
                                }
#endif
                                HDL_RESET_RX( pxFrameHdl );
