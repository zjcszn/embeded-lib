...
#define MBM_PDU_FUNC_RD_DISC_RESPSIZE( usNInputs ) ( UBYTE )( NBYTES_FOR_NINPUTS( usNInputs ) + 1 + 1 )
/* ----------------------- Type definitions ---------------------------------*/
                ...
                if( ( NULL != arubBufferOut ) && ( usNInputs > 0 ) && ( usNInputs <= MBM_PDU_FUNC_RD_DISCCNT_MAX ) )
                {
                    ...
                    if( bMBPPortLogIsEnabled( MB_LOG_DEBUG, MB_LOG_CORE ) )
                    {
                        vMBPPortLog( MB_LOG_DEBUG, MB_LOG_CORE,
                                     ...
                                     ( USHORT )pxIntHdl->ubIdx, usInputStartAddress, ( USHORT )usNInputs );
                    }
#endif
