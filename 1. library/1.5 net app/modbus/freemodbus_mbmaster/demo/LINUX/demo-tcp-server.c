...
main( int argc, char *argv[] )
{
    ...
    else if( MB_ENOERR != eMBPTCPServerInit( &xHandle, SERVER_LISTEN_IFACE, SERVER_PORT, xMBMMaster,
                                             eMBPTCPClientConnected, eMBTCPClientNewDataCB, eMBTCPClientDisconnected ) )
    {
        fprintf( stderr, "%s: Could not initialize modbus server on interface %s:%d.\n", argv[0], SERVER_LISTEN_IFACE,
                 SERVER_PORT );
        return -2;
    }
    else
