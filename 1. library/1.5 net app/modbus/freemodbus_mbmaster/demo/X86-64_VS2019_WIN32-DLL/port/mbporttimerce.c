    ...
    if( ( NULL != xTimerHdl ) && ( NULL != pbMBPTimerExpiredFN ) && ( MB_HDL_INVALID != xHdl ) )
    {
        if( !bIsRunning )
        {
            ...
            else
            {
                ...
                else if( 0 == ( xWinMMTimerId = timeSetEvent( TIMER_TIMEOUT, wTimerRes, xWinTimerEvHdl, 0,
                ...
                else if( NULL == ( xWinTimerThreadHdl = CreateThread( NULL, THREAD_STACKSIZE_MAX,
                                                                      prrvTimerHandlerThread, NULL, 0, &dwThreadId ) ) )
                {
                    ( void )CloseHandle( xWinTimerEvHdl );
...
eMBPTimerSetTimeout( xMBPTimerHandle xTimerHdl, USHORT usTimeOut1ms )
{
    ...
    if( MB_IS_VALID_HDL( pxTimerIntHdl, arxTimerHdls ) && ( usTimeOut1ms > 0 ) &&
        ( usTimeOut1ms != TIMER_TIMEOUT_INVALID ) )
    {
