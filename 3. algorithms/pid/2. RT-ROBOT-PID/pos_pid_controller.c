...
static int pos_pid_controller_reset(void *pid)
{
...
static int pos_pid_controller_destroy(void *pid)
{
    rt_free(pid);
    return 0;
}

static int pos_pid_controller_set_param(void *pid, controller_param_t param)
{
    pos_pid_controller_t pos_pid = (pos_pid_controller_t)pid;
...
static int pos_pid_controller_get_param(void *pid, controller_param_t param)
{
    pos_pid_controller_t pos_pid = (pos_pid_controller_t)pid;
...
static int pos_pid_controller_update(void *pid, float current_point)
{
    pos_pid_controller_t pos_pid = (pos_pid_controller_t)pid;

    if((GET_SYSTEM_MS() - pos_pid->last_time) < pos_pid->controller.sample_time)
    {
        LOG_D("PID waiting ... ");
        return 1;
    }
    pos_pid->last_time = GET_SYSTEM_MS();
    ...
    else if (pos_pid->last_out < pos_pid->minimum)
    {
...
pos_pid_controller_t pos_pid_controller_create(float kp, float ki, float kd, uint16_t sample_time)
{
...
int pos_pid_controller_set_kp(pos_pid_controller_t pid, float kp)
{
    RT_ASSERT(pid != NULL);
...
int pos_pid_controller_set_ki(pos_pid_controller_t pid, float ki)
{
    RT_ASSERT(pid != NULL);
...
int pos_pid_controller_set_kd(pos_pid_controller_t pid, float kd)
{
    RT_ASSERT(pid != NULL);
