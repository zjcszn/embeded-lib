...
#define rtos_calloc(num, size)      calloc(num, size)
...
fifo_queue_t fifo_queue_create(unsigned int bufsz) 
{
  ...
  fifo = (fifo_queue_t) rtos_malloc(sizeof(struct _fifo));
  if (fifo != NULL)
  {
...
/* warning: use _putc or _getc function , must check read or write length > 0 */
